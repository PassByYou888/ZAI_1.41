...
begin
  ...
  TComputeThread.RunP(nil, nil, procedure(Sender: TComputeThread)
    ...
    begin
      ...
      DoStatus('Learn引擎K维：%d', [PasAI.AI.C_Metric_Dim]);
      L_Engine := TLearn.CreateClassifier(TLearnType.ltKDT, PasAI.AI.C_Metric_Dim);
    end);
end;
  ...
  TComputeThread.RunP(nil, nil, procedure(Sender: TComputeThread)
    ...
    begin
      ...
      try
        ...
        // learn学习这一步可以保存成文件，不必每次学习
        DoStatus('Learn引擎正在学习Face度量', []);
        tmpLearn := TLearn.CreateClassifier(TLearnType.ltKDT, PasAI.AI.C_Metric_Dim);
        L_Engine.Clear;
        tk := GetTimeTick();
        if DNNThread_CheckBox.IsChecked then
          begin
            AI.Metric_ResNet_SaveToLearnEngine_DT(mdnn_stream, False, 2, imgL, tmpLearn)
          end
        else
          begin
            DoStatus('载入度量化神经网络 "%s"', [fn.Text]);
