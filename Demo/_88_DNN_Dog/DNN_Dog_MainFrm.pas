...
uses
  ...
  PasAI.AI, PasAI.AI.Common, PasAI.AI.TrainingTask, PasAI.AI.Editor,
    ...
    begin
      ...
      with TPas_AI_TrainingTask.OpenMemoryTask(WhereFileFromConfigure('dog_train_output_detector.OX')) do
        begin
          stream := TMS64.Create;
          // 从训练输出结果读取小狗检测器模型
          Read('dog_detector.svm_dnn_od', stream);
      ...
      with TPas_AI_TrainingTask.OpenMemoryTask(WhereFileFromConfigure('dog_train_output_metric.OX')) do
        begin
          ...
          stream := TMS64.Create;
          Read('dog_metric.Learn', stream);
          Metric_Learn := TLearn.CreateClassifier(ltKDT, PasAI.AI.C_Metric_Dim);
  ...
  if length(desc) > 0 then
    begin
      ...
      for i := 0 to length(desc) - 1 do
        begin
          // 按尺度抠图
          tmp := viewIntf[0].raster.BuildAreaOffsetScaleSpace(desc[i].R, PasAI.AI.C_Metric_Input_Size, PasAI.AI.C_Metric_Input_Size);
          // 计算抠图的度量化
...
begin
  ...
  scaleSpace_raster := det.Owner.raster.BuildAreaOffsetScaleSpace(det.R, PasAI.AI.C_Metric_Input_Size, PasAI.AI.C_Metric_Input_Size);
