...
uses
  System.SysUtils,
  PasAI.Core, PasAI.PascalStrings, PasAI.UPascalStrings, PasAI.UnicodeMixedLib, PasAI.DFE, PasAI.Parsing, PasAI.Expression, PasAI.OpCode, PasAI.Status,
...
begin
  if C40_Extract_CmdLine(TTextStyle.tsC, // 极简C4框架,方便测试
    ...
    begin
      DoStatus('第2个主线程ID: %d', [Core_Main_Thread_ID]); // 次主线底层的线程同步机制,也定义自己的灵活主线模型
...
begin
  ...
  // 第1个主线程是RTL, 第2个主线程是次主线
  // 次主线双主线程模型,已经全面测试通过,良好支持C4,SrossSocket,ICS8,ICS9,Indy,Synapse,DIOCP
  ...
  DoStatus('模拟次主线已经结束.');
  // 程序执行完成后,运行到这里,已经还原线程环境,再运行一次C4服务,这次是在第一个主线空间运行
  Second_Main();
