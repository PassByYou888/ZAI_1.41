...
type
  ...
  private
    ...
    procedure Do_Remove_First; // 多线程实例遍历滚动存储
  public
...
var
  ...
  Task_Num: Integer;
  Add_Num, Remove_Num, Parallel_Load_Num, For_Num, Remove_First_Num, Progress_Num: Int64;
  Sim_Running: Boolean;
...
procedure T_129_ZDB2_Thread_App_DemoForm.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  CanClose := not Sim_Running;
...
begin
  ...
  Remove_First_Num_Label.Caption := Format('滚存次数:%d', [Remove_First_Num]);
...
procedure T_129_ZDB2_Thread_App_DemoForm.Run_ButtonClick(Sender: TObject);
begin
  if Sim_Running then
    begin
      Sim_Running := False;
...
procedure T_129_ZDB2_Thread_App_DemoForm.Do_Remove_First;
begin
  Sim_Mem_DB.Remove_First_Data_For_All_Th_Engine(1024 * 1024);
  AtomInc(Remove_First_Num);
end;
