...
begin
  ...
  if MJPEG_Encodec = nil then
      MJPEG_Encodec := avcodec_find_encoder(AV_CODEC_ID_MJPEG);

  try
      Result := InternalOpenCodec(MJPEG_Encodec, Width, Height, 25, 1, 0, quantizerMin, quantizerMax, 1024 * 1024);
  except
      Result := False;
  end;
end;
