...
type
  ...
  public
    ...
    destructor Destroy; override;
    procedure DoDraw(d: TDrawEngine; Draw_Box_: TRectV2); virtual;
  end;
...
procedure TPictureViewerData.DoDraw(d: TDrawEngine; Draw_Box_: TRectV2);
begin
  ...
  if showPicture_ then
    for j := 0 to Count - 1 do
      begin
        sData := Items[j];
        if (sData.Enabled) and (not sData.Raster.IsEmpty) then
            sData.DoDraw(FDrawEng, sData.ScreenBox);
      end;
