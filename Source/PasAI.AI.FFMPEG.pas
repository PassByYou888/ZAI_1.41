...
uses PasAI.Core, PasAI.PascalStrings, PasAI.UPascalStrings, PasAI.MemoryRaster, PasAI.MemoryStream, PasAI.Notify,
  PasAI.AI, PasAI.FFMPEG, PasAI.FFMPEG.Reader;
...
begin
  ...
  while True do
    begin
      ...
      else if Assigned(OnAI_OpenFFMPEGVideoProcessor_M) then
          OnAI_OpenFFMPEGVideoProcessor_M(thSender, Processor, ProcessStop)
      else if Assigned(OnAI_OpenFFMPEGVideo_Processor_P) then
  ...
  else if Assigned(OnAI_OpenFFMPEGVideoProcessorDone_M) then
      OnAI_OpenFFMPEGVideoProcessorDone_M(thSender, Processor)
  else if Assigned(OnAI_OpenFFMPEGVideo_ProcessorDone_P) then
...
begin
  ...
  TCompute.RunM(nil, nil, Reader.ComputeThread_Run);
end;
