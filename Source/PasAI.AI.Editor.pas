...
{$ENDIF FPC}
  ...
  PasAI.Learn.KDTree, PasAI.Learn.Type_LIB, PasAI.Learn,
...
type
  ...
  TEditorGeometryList_ = TGenericsList<TEditorGeometry>;
  TEditorSegmentationMaskList_ = TGenericsList<TEditorSegmentationMask>;
  TEditorImageDataList_ = TGenericsList<TEditorImageData>;
  TEditorDetectorDefineList = TGenericsList<TEditorDetectorDefine>;
  TEditorDetectorDefinePartList = TGenericsList<PVec2>;

  TEditor_Num_Hash_Pool_ = TString_Big_Hash_Pair_Pool<Integer>;

  TEditor_Num_Hash_Pool = class(TEditor_Num_Hash_Pool_)
  public
  ...
  TEditorDetectorDefine = class(TCore_Object_Intermediate)
  private
    FIndex: Integer;
  ...
  TEditorGeometryList = class(TEditorGeometryList_)
  public
    Owner: TEditorImageData;
    constructor Create;
  ...
  TEditorSegmentationMask = class(TCore_Object_Intermediate)
  protected
  ...
  TEditorSegmentationMaskList = class(TEditorSegmentationMaskList_)
  public
    Owner: TEditorImageData;
  ...
  TEditorDetectorDefine_Overlap_ = TBigList<TEditorDetectorDefine>;

  TEditorDetectorDefine_Overlap = class(TEditorDetectorDefine_Overlap_)
  public
  ...
  TEditorDetectorDefine_Overlap_Tool_ = TBigList<TEditorDetectorDefine_Overlap>;

  TEditorDetectorDefine_Overlap_Tool = class(TEditorDetectorDefine_Overlap_Tool_)
  public
  ...
  TEditorImageData = class(TCore_Object_Intermediate)
  private
    FIndex: Integer;
    FOP_RT: TOpCustomRunTime;
    FOP_RT_RunDeleted: Boolean;
    FOP_RT_Run_Add_Image_List: TEditorImageDataList_;
    { register op }
  ...
  TEditor_Box_Cluster_Data_ = record
    V: TLVec;
    N: Integer;
  end;

  TEditor_Box_Cluster_Pool_ = TBigList<TEditor_Box_Cluster_Data_>;

  TEditor_Box_Cluster_Pool = class(TEditor_Box_Cluster_Pool_)
  private
    function Do_Sort_(var L, R: TEditor_Box_Cluster_Data_): Integer;
  public
    procedure Sort();
  end;

  TEditorImageDataList = class(TEditorImageDataList_)
  public
    FreeImgData: Boolean;
    ...
    procedure Remove_Empty_Image();
    function Compute_Detector_KMean_Cluster(num_clusers, FitX, FitY: Integer): TEditor_Box_Cluster_Pool; overload;
    function Compute_Detector_KMean_Cluster(num_clusers: Integer): TEditor_Box_Cluster_Pool; overload;
...
var
  p: TEditor_Num_Hash_Pool_.PValue;
begin
...
var
  __repeat__: TEditor_Num_Hash_Pool_.TRepeat___;
begin
...
begin
  ...
  if FViewerRaster = nil then
    begin
      ...
      TCompute.RunM_NP(BuildViewerTh);
    end;
...
procedure TEditorImageData.CheckAndReg_Param_OpRT;
begin
  if FOP_RT <> nil then
      exit;
  ...
  { condition on image }
  FOP_RT.Reg_Param_OpM('Index', 'Index(): Image Index', OP_Image_GetIndex)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('Width', 'Width(): Image Width', OP_Image_GetWidth)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('Height', 'Height(): Image Height', OP_Image_GetHeight)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('Det', 'Det(): Detector define of Count', OP_Image_GetDetector)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('Detector', 'Detector(): Detector define of Count', OP_Image_GetDetector)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('DetNum', 'DetNum(): Detector define of Count', OP_Image_GetDetector)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('Geo', 'Geo(): geometry define of Count', OP_Image_GetGeometry)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('Geometry', 'Geometry(): geometry define of Count', OP_Image_GetGeometry)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('GeoNum', 'GeoNum(): geometry define of Count', OP_Image_GetGeometry)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('Seg', 'Seg(): segmentation define of Count', OP_Image_GetSegmentation)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('Segmentation', 'Segmentation(): segmentation define of Count', OP_Image_GetSegmentation)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('SegNum', 'SegNum(): segmentation define of Count', OP_Image_GetSegmentation)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('IsTest', 'IsTest(): image is test', OP_Image_IsTest)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('FileInfo', 'FileInfo(): image file info', OP_Image_FileInfo)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('FindAllLabel', 'FindAllLabel(filter): num; return found label(det,geo,seg) num > 0', OP_Image_FindLabel)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('FindAllLabel', 'FindAllLabel(filter): num; return found label(det,geo,seg) num > 0', OP_Image_FindLabel)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('MD5', 'MD5(): return rasterization md5', OP_Image_MD5)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('Gradient_MD5', 'Gradient_MD5(): return rasterization Level 16 Gradient md5', OP_Image_Gradient_L16_MD5)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('Random_Str', 'Random_Str(): return only one random string.', OP_Image_Random_Str)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('RandomStr', 'RandomStr(): return only one random string.', OP_Image_Random_Str)^.Category := 'AI Editor';

  { condition on detector }
  FOP_RT.Reg_Param_OpM('Label', 'Label(name): num; return Label num', OP_Detector_GetLabel)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('GetLabel', 'GetLabel(name): num; return Label num', OP_Detector_GetLabel)^.Category := 'AI Editor';

  { process on image }
  FOP_RT.Reg_Param_OpM('Delete', 'Delete(): Delete image', OP_Image_Delete)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('Scale', 'Scale(k:Float): scale image', OP_Image_Scale)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('ReduceMemory', 'ReduceMemory(k:Float): scale image', OP_Image_Scale)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('FitScale', 'FitScale(Width, Height): fitscale image', OP_Image_FitScale)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('FixedScale', 'FixedScale(Res): fitscale image', OP_Image_FixedScale)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('InnerFitScale', 'InnerFitScale(scale_x, scale_y): inner fit scale', OP_Image_InnerFitScale)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('InnerFit', 'InnerFit(scale_x, scale_y): inner fit scale', OP_Image_InnerFitScale)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('SwapRB', 'SwapRB(): swap red blue channel', OP_Image_SwapRB)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('SwapBR', 'SwapRB(): swap red blue channel', OP_Image_SwapRB)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('Gray', 'Gray(): Convert image to grayscale', OP_Image_Gray)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('Grayscale', 'Grayscale(): Convert image to grayscale', OP_Image_Gray)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('Sharpen', 'Sharpen(): Convert image to Sharpen', OP_Image_Sharpen)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('HistogramEqualize', 'HistogramEqualize(): Convert image to HistogramEqualize', OP_Image_HistogramEqualize)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('he', 'he(): Convert image to HistogramEqualize', OP_Image_HistogramEqualize)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('NiceColor', 'NiceColor(): Convert image to HistogramEqualize', OP_Image_HistogramEqualize)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('RemoveRedEye', 'RemoveRedEye(): Remove image red eye', OP_Image_RemoveRedEyes)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('RemoveRedEyes', 'RemoveRedEyes(): Remove image red eye', OP_Image_RemoveRedEyes)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('RedEyes', 'RedEyes(): Remove image red eye', OP_Image_RemoveRedEyes)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('RedEye', 'RedEye(): Remove image red eye', OP_Image_RemoveRedEyes)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('Sepia', 'Sepia(Depth): Convert image to Sepia', OP_Image_Sepia)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('Blur', 'Blur(radius): Convert image to Blur', OP_Image_Blur)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('CalibrateRotate', 'CalibrateRotate(): Using Hough transform to calibrate rotation', OP_Image_CalibrateRotate)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('DocumentAlignment', 'DocumentAlignment(): Using Hough transform to calibrate rotation', OP_Image_CalibrateRotate)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('DocumentAlign', 'DocumentAlign(): Using Hough transform to calibrate rotation', OP_Image_CalibrateRotate)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('DocAlign', 'DocAlign(): Using Hough transform to calibrate rotation', OP_Image_CalibrateRotate)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('AlignDoc', 'AlignDoc(): Using Hough transform to calibrate rotation', OP_Image_CalibrateRotate)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('FlipHorz', 'FlipHorz(): FlipHorz', OP_Image_FlipHorz)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('FlipVert', 'FlipVert(): FlipVert', OP_Image_FlipVert)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('SetTest', 'SetTest(bool): change test', OP_Image_SetTest)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('SetFileInfo', 'SetFileInfo(string): change file info', OP_Image_SetFileInfo)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('ProjectionImageAs', 'ProjectionImageAs(left,top,right,bottom,angle,width,height): projection as new image', OP_Image_ProjectionImageAs)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('SaveToFile', 'SaveToFile(file name): save Rasterization to file.', OP_Image_SaveToFile)^.Category := 'AI Editor';

  { process on detector }
  FOP_RT.Reg_Param_OpM('SetLab', 'SetLab(newLabel name): new Label name', OP_Detector_SetLabel)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('SetLabel', 'SetLabel(newLabel name): new Label name', OP_Detector_SetLabel)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('DefLab', 'DefLab(newLabel name): new Label name', OP_Detector_SetLabel)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('DefLabel', 'DefLabel(newLabel name): new Label name', OP_Detector_SetLabel)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('DefineLabel', 'DefineLabel(newLabel name): new Label name', OP_Detector_SetLabel)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('RemoveNoDefineDetector', 'RemoveNoDefineDetector(): clean detector box from no define.', OP_Detector_ClearNoDefine)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('RemoveNoMatchDetector', 'RemoveNoMatchDetector(label): clean detector box from none match', OP_Detector_NoMatchClear)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('ClearDetector', 'ClearDetector(): clean detector box', OP_Detector_ClearDetector)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('ClearDet', 'ClearDet(): clean detector box', OP_Detector_ClearDetector)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('KillDetector', 'KillDetector(): clean detector box', OP_Detector_ClearDetector)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('KillDet', 'KillDet(): clean detector box', OP_Detector_ClearDetector)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('DeleteDetector', 'DeleteDetector(Maximum reserved box, x-scale, y-scale): delete detector box', OP_Detector_DeleteDetector)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('DeleteRect', 'DeleteRect(Maximum reserved box, x-scale, y-scale): delete detector box', OP_Detector_DeleteDetector)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('RemoveInvalidDetectorFromPart', 'RemoveInvalidDetectorFromPart(fixedPartNum): delete detector box from Part num', OP_Detector_RemoveInvalidDetectorFromPart)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('RemoveInvalidDetectorFromSPNum', 'RemoveInvalidDetectorFromSPNum(fixedPartNum): delete detector box from Part num', OP_Detector_RemoveInvalidDetectorFromPart)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('RemoveDetPart', 'RemoveDetPart(): remove detector define part', OP_Detector_RemovePart)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('RemoveMinArea', 'RemoveMinArea(width, height): remove detector from minmize area', OP_Detector_RemoveMinArea)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('ResetSequence', 'ResetSequence(): reset sequence data from detector', OP_Detector_Reset_Sequence)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('SetLabelFromArea', 'SetLabelFromArea(minArea, maxArea, label): set label from area', OP_Detector_SetLabelFromArea)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('RemoveOutEdgeBox', 'RemoveOutEdgeBox(): remove box from out edge/intersect edge', OP_Detector_RemoveOutEdge)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('RemoveOverlap', 'RemoveOverlap() or RemoveOverlap(Distance): remove overlap box', OP_Detector_RemoveOverlap)^.Category := 'AI Editor';

  { process on geometry }
  FOP_RT.Reg_Param_OpM('ClearGeometry', 'ClearGeometry(): clean geometry', OP_Geometry_ClearGeometry)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('ClearGeo', 'ClearGeo(): clean geometry', OP_Geometry_ClearGeometry)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('KillGeometry', 'KillGeometry(): clean geometry', OP_Geometry_ClearGeometry)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('KillGeo', 'KillGeo(): clean geometry', OP_Geometry_ClearGeometry)^.Category := 'AI Editor';

  { process on Segmentation mask }
  FOP_RT.Reg_Param_OpM('ClearSegmentationMask', 'ClearSegmentationMask(): clean segmentation mask', OP_SegmentationMask_ClearSegmentationMask)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('ClearSeg', 'ClearSeg(): clean segmentation mask', OP_SegmentationMask_ClearSegmentationMask)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('KillSegmentationMask', 'KillSegmentationMask(): clean segmentation mask', OP_SegmentationMask_ClearSegmentationMask)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('KillSeg', 'KillSeg(): clean segmentation mask', OP_SegmentationMask_ClearSegmentationMask)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('Replace', 'Replace(OldPattern, NewPattern): replace detector, geometry, segment label', OP_Replace)^.Category := 'AI Editor';

  FOP_RT.Reg_Param_OpM('S2PY', 'S2PY(): FastGBK translation Simplified of Pinyin', OP_S2PY)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('S2PY2', 'S2PY2(): GBK translation Simplified of Pinyin', OP_S2PY2)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('S2T', 'S2T(): Simplified to Traditional', OP_S2T)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('S2H', 'S2H(): Simplified to Hongkong Traditional (built-in vocabulary conversion)', OP_S2H)^.Category := 'AI Editor';
  FOP_RT.Reg_Param_OpM('T2S', 'T2S(): Traditional to Simplified (built-in vocabulary conversion)', OP_T2S)^.Category := 'AI Editor';
...
function TEditorImageData.OP_Image_InnerFitScale(var Param: TOpParam): Variant;
begin
  if not Raster.Empty then
    begin
...
begin
  ...
  FOP_RT := nil;
  FOP_RT_RunDeleted := False;
  FOP_RT_Run_Add_Image_List := TEditorImageDataList_.Create;
  CreateTime := umlNow();
...
procedure TEditorImageData.InnerFitScale(Scale_X, Scale_Y: TGeoFloat);
var
  sour, dest: TRectV2;
  nR: TPasAI_Raster;
  i, j, k: Integer;
...
begin
  ...
  // compute geometry projection
  for i := 0 to GeometryList.Count - 1 do
    begin
      geo := GeometryList[i];
      for j := 0 to geo.Surround.Count - 1 do
          geo.Surround[j]^ := RectProjection(dest, nR.BoundsRectV2, geo.Surround[j]^);
      for j := 0 to length(geo.Collapses) - 1 do
        for k := 0 to geo.Collapses[j].Count - 1 do
            geo.Collapses[j][k]^ := RectProjection(dest, nR.BoundsRectV2, geo.Collapses[j][k]^);
    end;
  ...
  for i := 0 to SegmentationMaskList.Count - 1 do
    begin
      ...
      if seg.FViewerRaster <> nil then
        begin
          // rebuild segmentation view raster
          DisposeObjectAndNil(seg.FViewerRaster);
        end;
  ...
  DisposeObject(nR);
  // compute draw box
  RasterDrawRect := FitRect(Scale_X, Scale_Y, RasterDrawRect);
end;
...
begin
  ...
  needReset := (MachineProcess is TPas_AI_TECH_2022_Machine_ZMetric_V2) or
    (MachineProcess is TPas_AI_TECH_2022_Machine_YOLO_X) or
    (ai_img.DetectorDefineList.Count <> DetectorDefineList.Count);
...
function TEditor_Box_Cluster_Pool.Do_Sort_(var L, R: TEditor_Box_Cluster_Data_): Integer;
begin
  Result := CompareInteger(L.N, R.N);
end;

procedure TEditor_Box_Cluster_Pool.Sort;
begin
  Sort_M(Do_Sort_);
end;
...
procedure TEditorImageDataList.Remove_Empty_Image;
var
...
function TEditorImageDataList.Compute_Detector_KMean_Cluster(num_clusers, FitX, FitY: Integer): TEditor_Box_Cluster_Pool;
var
  KDL: TKDTreeDataList;
  rnd: TMT19937Random;
  KD: TKDTree;
...
begin
  ...
  Result := TEditor_Box_Cluster_Pool.Create;
...
function TEditorImageDataList.Compute_Detector_KMean_Cluster(num_clusers: Integer): TEditor_Box_Cluster_Pool;
var
  KDL: TKDTreeDataList;
  rnd: TMT19937Random;
  KD: TKDTree;
...
function TEditorImageDataList.Compute_YOLO_Anchors(var T8_, T16_, T32_: U_String): Boolean;
var
  L: TEditor_Box_Cluster_Pool;
